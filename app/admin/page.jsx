...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const summaryData = summaryResponse.data?.success ? summaryResponse.data.data : null;
      ...
      // Fetch stores data
      const storesResponse = await api.get('/getAllStores');
      const storesData = storesResponse.data?.stores || [];

      // Combine data from all endpoints
      setDashboardData({
        products: summaryData?.totalProducts || dashboardResponseData?.products || 0,
        revenue: summaryData?.totalRevenue || dashboardResponseData?.revenue || 0,
        orders: summaryData?.totalOrders || dashboardResponseData?.orders || 0,
        stores: summaryData?.totalStores || dashboardResponseData?.stores || 0,
        users: dashboardResponseData?.users || 0,
        allOrders: dashboardResponseData?.allOrders || [],
      });
      ...
      console.error('Error fetching dashboard data:', errorMessage, error);
      setDashboardData({
        products: 0,
        revenue: 0,
        orders: 0,
    ...
    <DashboardLayout activePage="dashboard">
      <div className="p-6">
        ...
        {activeView === 'overview' ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                {dashboardData.allOrders?.length > 0 ? (
                  <OrdersAreaChartImproved allOrders={dashboardData.allOrders} />
                ) : (
          ...
          <div className="mt-6">
            {/* We'll keep the advanced analytics component as is for now */}
            {/* In a real implementation, you might want to import and use a separate component */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              <h2 className="text-xl font-semibold text-slate-800 mb-4">Advanced Analytics</h2>
              <p className="text-slate-600">Advanced analytics features would be implemented here.</p>
            </div>
          </div>
        )}
