...
api.interceptors.response.use(
  (response) => {
    ...
    console.log("API Response:", response.status, response.config.url, {
      dataPreview: response.data ? JSON.stringify(response.data).substring(0, 100) : 'none'
    });
    return response;
  },
  (error) => {
    ...
    if (!error.response) {
      ...
      error.message = "Network Error: Check your internet connection or API server availability";
    } else if (error.response?.status === 400) {
      console.error("Bad Request - Check request parameters");
      error.message = "Bad Request - Check request parameters";
    } else if (error.response?.status === 401) {
      console.error("Unauthorized - Check authentication token");
      error.message = "Unauthorized - Check authentication token";
    } else if (error.response?.status === 403) {
      console.error("Access denied to endpoint");
      error.message = "Access denied to endpoint";
    } else if (error.response?.status === 404) {
      console.error("Endpoint not found - Check if the API endpoint exists");
      error.message = "Endpoint not found - Check if the API endpoint exists";
    } else if (error.response?.status === 500) {
      console.error("Internal server error");
      error.message = "Internal server error";
    }
    
    // If we get an empty error response, provide more context
    if (error.message === "API Error: {}" || (error.response && Object.keys(error.response.data || {}).length === 0)) {
      console.error("Received empty error response. This might indicate an authentication issue or server misconfiguration.");
      error.message = "Received empty error response from server. Please check authentication token and server configuration.";
    }
...
api.getRevenueTrend = (storeId) => {
  ...
  // For admin users, use the admin dashboard endpoint
  // For store users, use the store-specific endpoint
  const endpoint = storeId ? `/dashboard/store/${storeId}` : '/admin/dashboard';
