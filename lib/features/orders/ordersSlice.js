...
export const getStoreInvoices = createAsyncThunk(
    ...
    async (_, { getState, rejectWithValue }) => {
        try {
            ...
            const url = 'https://go-cart-1bwm.vercel.app/api/kiro/order/invoices'
            ...
            if (error.name === 'AbortError') {
                ...
                return rejectWithValue('Unable to connect to the server. Please check your internet connection and ensure the API server is running. The API endpoint being accessed is: https://go-cart-1bwm.vercel.app/api/kiro/order/invoices');
            } else if (error.message.includes('CORS') || error.message.includes('Failed to fetch')) {
                console.error('CORS error or network issue:', error);
                return rejectWithValue('Unable to connect to the server. This may be due to CORS restrictions or network issues. Please try again later or contact support. The API endpoint being accessed is: https://go-cart-1bwm.vercel.app/api/kiro/order/invoices');
            }
            return rejectWithValue(error.message)
        }
    }
