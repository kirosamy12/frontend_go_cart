...
const getCart = createAsyncThunk('cart/getCart', async (userId, { rejectWithValue }) => {
  try {
...
const createCart = createAsyncThunk('cart/createCart', async (userData, { rejectWithValue }) => {
  try {
    const res = await axios.post(`/api/cart`, userData);
    return res.data;
  } catch (err) {
    return rejectWithValue(err.response?.data || err.message);
  }
...
const updateCart = createAsyncThunk('cart/updateCart', async (data, { rejectWithValue }) => {
  try {
    const res = await axios.patch(`/api/cart/${data.id}`, data);
    return res.data;
  } catch (err) {
    return rejectWithValue(err.response?.data || err.message);
  }
