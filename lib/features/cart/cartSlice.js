...
export const addToCartAsync = createAsyncThunk(
    'cart/addToCartAsync',
    async ({ productId, quantity = 1, selectedColor, selectedSize }, { getState, rejectWithValue }) => {
        try {
            const { auth } = getState()
...
export const removeFromCartAsync = createAsyncThunk(
    ...
    async (productId, { getState, rejectWithValue }) => {
        try {
            ...
            const response = await fetch(`https://go-cart-1bwm.vercel.app/api/removeFromCart/${productId}`, {
                method: 'PATCH',
                headers: {
                    'token': auth.token,
                    'Content-Type': 'application/json',
...
export const createOrder = createAsyncThunk(
    ...
    async (orderData, { getState, rejectWithValue }) => {
        try {
            ...
            const numericShippingCost = isNaN(orderData.shippingCost) ? 0 : Number(orderData.shippingCost);
    ...
    reducers: {
        addToCart: (state, action) => {
            ...
            if (state.cartItems[productId]) {
                if (typeof state.cartItems[productId] === 'object') {
        ...
        builder
            ...
            .addCase(getCart.fulfilled, (state, action) => {
                ...
                if (action.payload) {
                    ...
                    state.total = action.payload.total || Object.values(state.cartItems).reduce((sum, qty) => typeof qty === 'object' ? qty.quantity : qty, 0);
                }
            })
            ...
            .addCase(createCart.fulfilled, (state, action) => {
                ...
                // Assuming the cart data has items as array of {productId, quantity, selectedColor} and total
                if (action.payload) {
                    if (action.payload.items && Array.isArray(action.payload.items)) {
